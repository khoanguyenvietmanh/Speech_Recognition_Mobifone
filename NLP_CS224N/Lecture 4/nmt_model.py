...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTMCell(embed_size + self.hidden_size, hidden_size=self.hidden_size, bias=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        last_hidden = torch.cat([last_hidden[0], last_cell[1]], dim=1)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        List_Y_split = torch.split(Y, 1)

        for Y_split in List_Y_split:
            Y_split = torch.squeeze(Y_split, dim=0)

            Ybar_t = torch.cat([Y_split, o_prev], dim=-1)

            (cell, state, o_t) = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
