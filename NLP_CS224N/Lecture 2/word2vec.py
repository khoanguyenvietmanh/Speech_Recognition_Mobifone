...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    inner_product = np.dot(outsideVectors, centerWordVec)
    y_hat = softmax(inner_product)[:, np.newaxis]
    loss = - np.log(y_hat[outsideWordIdx])

    gradCenterVec = - outsideVectors[outsideWordIdx] + np.sum(outsideVectors * y_hat, axis = 0)

    gradOutsideVecs = np.dot(y_hat, centerWordVec[:, np.newaxis].T)
...
def negSamplingLossAndGradient(
    ...
    scores = np.dot(outsideVectors[indices], centerWordVec)[:, np.newaxis]
    ...
    gradFromCorpus = outsideVectors[outsideWordIdx] * (probFromCorpus - 1)

    gradNotFromCorpus = np.sum(outsideVectors[negSampleWordIndices] * (1 - probNotFromCorpus), axis=0)

    gradCenterVec = gradFromCorpus + gradNotFromCorpus

    gradOutsideVecs = np.zeros(outsideVectors.shape)

    gradOutsideVecs[outsideWordIdx] += centerWordVec * (probFromCorpus - 1)

    for i, neg_indice in enumerate(negSampleWordIndices):
        gradOutsideVecs[neg_indice] += centerWordVec * (1 - probNotFromCorpus[i])
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWordIndice in outsideWordIndices:
        cur_loss, cur_gradCentVec, cur_gradOutSideVecs = word2vecLossAndGradient(centerWordVec, outsideWordIndice, outsideVectors, dataset)

        loss += cur_loss

        gradCenterVecs[currentCenterWordIdx] += cur_gradCentVec
